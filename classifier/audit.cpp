...
namespace classifier {
    void audit(const std::filesystem::path& dataset, const std::filesystem::path& model, const std::atomic_bool& interrupted) {
        ...
        if (!provenance.is_null())
            for (const auto& record : provenance.at("records")) records[record.at("id").get<std::string>()] = record;
        nlohmann::json report{{"version", 1}, {"dataset", path_utf8(root)}, {"fingerprint", content}, {"model", path_utf8(model_path)}, {"model_sha256", model_hash}, {"step", provenance.is_null() ? nlohmann::json(nullptr) : nlohmann::json(std::stoi(network.metadata.at("step").get<std::string>()))}, {"training_fingerprint", provenance.is_null() ? nlohmann::json(nullptr) : provenance.at("fingerprint")}, {"created_utc", std::format("{:%FT%TZ}", std::chrono::floor<std::chrono::seconds>(std::chrono::system_clock::now()))}, {"classes", network.classes}, {"threshold", loaded.threshold}, {"rows", nlohmann::json::array()}};
        ...
        for (const auto& [path, label] : paths) {
            ...
            report["rows"].push_back({{"path", id}, {"label", label}, {"predicted", predicted}, {"scores", scores}, {"split", split}, {"rgb_sha256", digest}, {"width", image.width}, {"height", image.height}, {"accepted", result.passed}});
