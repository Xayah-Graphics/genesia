...
module genesia.editor.graphics.interop;
import genesia.compute.inference;
import std;
import vulkan;

namespace genesia::editor {
    Interop::Interop(graphics::Device& device) : device{device} {
        // First-use kernel loading must finish before sampling and preview overlap.
        compute::check(prepare_rgba());
        ...
        for (auto& slot : slots) {
            ...
            compute::check(cudaImportExternalSemaphore(std::out_ptr(slot.semaphore), &description));
        }
    }
    ...
    void Interop::prepare(const int width, const int height, const ::cuda::stream_ref stream) {
        ...
        for (auto& slot : slots) {
            ...
            if (slot.value) {
                ...
                compute::check(cudaWaitExternalSemaphoresAsync(&semaphore, &wait, 1, stream.get()));
            }
            stream.sync();
            slot.pixels.reset();
            slot.memory.reset();
            ...
            compute::check(cudaImportExternalMemory(std::out_ptr(slot.memory), &description));
            const cudaExternalMemoryBufferDesc mapping{0, slot.buffer.allocation_size, 0};
            compute::check(cudaExternalMemoryGetMappedBuffer(std::out_ptr<void*>(slot.pixels), slot.memory.get(), &mapping));
        }
    }
    ...
    std::uint64_t Interop::publish(const std::uint8_t* pixels, const int width, const int height, const ::cuda::stream_ref stream, const std::size_t index) {
        ...
        compute::check(cudaSignalExternalSemaphoresAsync(&semaphore, &signal, 1, stream.get()));
        return slot.value;
    }
} // namespace genesia::editor
