...
namespace genesia::sdxl {
    VaeAttention::VaeAttention(Checkpoint& source, const std::string& prefix)
        : norm{source.norm(prefix + ".norm", neural::Scalar::bf16, 1.0e-6F)},
          qkv{source.qkv(std::array{prefix + ".q", prefix + ".k", prefix + ".v"}, neural::Scalar::bf16, true)},
          projection{source.convolution(prefix + ".proj_out", neural::Scalar::bf16, 1, 0)} {}
