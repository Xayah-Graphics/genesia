...
module genesia.models.sdxl.vae;
import std;
...
namespace genesia::sdxl {
    ...
    void VaeAttention::forward(const compute::TensorView current, compute::InferenceRuntime& runtime, const Workspace& scratch) const {
        ...
        const compute::TensorView packed     = scratch.hidden.reshape(1, current.h, current.w, 1536, compute::Scalar::bf16);
        const compute::TensorView attended   = scratch.output.reshape(1, current.h, current.w, 512, compute::Scalar::bf16);
    ...
    VAE::VAE(Checkpoint& source) : attention{source, "first_stage_model.decoder.mid.attn_1"} {
        ...
        for (int level = 0; level < 4; ++level) {
            ...
            if (level < 3) up[level].resize = source.convolution(prefix + ".upsample.conv", compute::Scalar::bf16);
        }
        norm   = source.norm(root + "norm_out", compute::Scalar::bf16, 1.0e-6F);
        output = source.convolution(root + "conv_out", compute::Scalar::bf16);
    }
    ...
    void VAE::forward(const compute::TensorView result, const compute::TensorView latent, compute::TensorView current, compute::InferenceRuntime& runtime, const Workspace& scratch) const {
        ...
        current.w      = latent.w;
        current.c      = 512;
        current.scalar = compute::Scalar::bf16;
        ...
        for (const auto& stage : up) {
            ...
            if (stage.resize.weight.data) {
                const compute::TensorView resized = scratch.combined.reshape(1, current.h * 2, current.w * 2, current.c, compute::Scalar::bf16);
                compute::kernels::resize(runtime.stream, resized.data, current.data, 1, current.h, current.w, current.c, 2);
                current.h *= 2;
                current.w *= 2;
                runtime.convolution(current, resized, stage.resize);
            }
        }
        const compute::TensorView activated = scratch.normalized.reshape(1, current.h, current.w, 128, compute::Scalar::bf16);
        runtime.group_norm(activated, current, norm, scratch.statistics, true);
