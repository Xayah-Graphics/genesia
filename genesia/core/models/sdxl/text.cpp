...
module genesia.models.sdxl.text;
import std;
...
namespace genesia::sdxl {
    Clip::Clip(Checkpoint& source, const bool is_large) : width{is_large ? 1280 : 768}, large{is_large}, empty{source.runtime.stream, ::cuda::device_default_memory_pool(source.runtime.stream.device()), 77uz * width, ::cuda::no_init} {
        ...
        token                  = source.tensor(root + (large ? "token_embedding.weight" : "embeddings.token_embedding.weight"), compute::Scalar::f16);
        position               = source.tensor(root + (large ? "positional_embedding" : "embeddings.position_embedding.weight"), compute::Scalar::f16);
        for (int index = 0; index < (large ? 32 : 11); ++index) {
            ...
            ClipBlock block;
            block.norm1 = source.norm(prefix + (large ? "ln_1" : "layer_norm1"), compute::Scalar::f16);
            if (large) block.qkv = {source.tensor(prefix + "attn.in_proj_weight", compute::Scalar::f16), source.tensor(prefix + "attn.in_proj_bias", compute::Scalar::f16)};
            else block.qkv = source.qkv(std::array{prefix + "self_attn.q_proj", prefix + "self_attn.k_proj", prefix + "self_attn.v_proj"}, compute::Scalar::f16, true);
            block.projection = source.linear(prefix + (large ? "attn.out_proj" : "self_attn.out_proj"), compute::Scalar::f16);
            block.norm2      = source.norm(prefix + (large ? "ln_2" : "layer_norm2"), compute::Scalar::f16);
            block.expand     = source.linear(prefix + (large ? "mlp.c_fc" : "mlp.fc1"), compute::Scalar::f16);
            block.contract   = source.linear(prefix + (large ? "mlp.c_proj" : "mlp.fc2"), compute::Scalar::f16);
            blocks.push_back(block);
        }
        if (large) {
            final_norm = source.norm(root + "ln_final", compute::Scalar::f16);
            projection = {source.tensor(root + "text_projection", compute::Scalar::f16, false, true), {}};
        }
    }

    void Clip::forward(const compute::TensorView sequence, const std::int32_t* ids, compute::InferenceRuntime& runtime, const Workspace& scratch) const {
        ...
        const compute::TensorView normalized = scratch.normalized.reshape(sequence.n, 1, 77, width, compute::Scalar::f16);
        const compute::TensorView qkv        = scratch.hidden.reshape(sequence.n, 1, 77, width * 3, compute::Scalar::f16);
        const compute::TensorView attention  = scratch.output.reshape(sequence.n, 1, 77, width, compute::Scalar::f16);
        const compute::TensorView expanded   = scratch.hidden.reshape(sequence.n, 1, 77, width * 4, compute::Scalar::f16);
        for (int index = 0; index < (large ? 31 : 11); ++index) {
            ...
            compute::TensorView key   = qkv;
            compute::TensorView value = qkv;
            ...
            compute::kernels::activation(runtime.stream, expanded.data, expanded.data, expanded.elements(), large ? 2 : 1);
            runtime.linear(sequence, expanded, block.contract, sequence);
        }
    }

    void Clip::pool(const compute::TensorView pooled, const compute::TensorView sequence, const std::int32_t* eos, compute::InferenceRuntime& runtime, const Workspace& scratch) const {
        // Only the first block's EOS output contributes to pooled conditioning.
        const auto& last                   = blocks.back();
        const compute::TensorView norm     = scratch.normalized.reshape(1, 1, 77, width, compute::Scalar::f16);
        const compute::TensorView kv       = scratch.hidden.reshape(1, 1, 77, width * 2, compute::Scalar::f16);
        const compute::TensorView residual = scratch.output.reshape(1, 1, 1, width, compute::Scalar::f16);
        const compute::TensorView query    = scratch.shortcut.reshape(1, 1, 1, width, compute::Scalar::f16);
        const compute::TensorView attended = scratch.combined.reshape(1, 1, 1, width, compute::Scalar::f16);
        compute::TensorView first          = sequence;
        first.n                            = 1;
        runtime.layer_norm(norm, first, last.norm1);
        compute::Linear q_layer  = last.qkv;
        q_layer.weight.w         = width;
        q_layer.bias.c           = width;
        compute::Linear kv_layer = last.qkv;
        ...
        compute::TensorView value = kv;
        value.data                = static_cast<__half*>(kv.data) + width;
        runtime.attention(attended, query, kv, value, width / 64, width, width * 2, true, eos);
        const compute::TensorView single = scratch.normalized.reshape(1, 1, 1, width, compute::Scalar::f16);
        ...
        const compute::TensorView hidden = scratch.hidden.reshape(1, 1, 1, width * 4, compute::Scalar::f16);
        runtime.linear(hidden, single, last.expand);
        compute::kernels::activation(runtime.stream, hidden.data, hidden.data, hidden.elements(), 2);
    ...
    void Clip::prepare_empty(compute::InferenceRuntime& runtime, const Workspace& scratch) {
        ...
        forward(compute::TensorView{empty.data(), 1, 1, 77, width, compute::Scalar::f16}, ids.data(), runtime, scratch);
        runtime.stream.sync();
    }
    Encoding Clip::encode(const Tokens& positive, const Tokens& negative, compute::InferenceRuntime& runtime, const Workspace& scratch) const {
        ...
        const compute::TensorView sequence{result.sequence.data(), positive.chunks + negative.chunks, 1, 77, width};
