...
namespace genesia::convnext {
    __device__ inline float read(compute::TensorView t, std::size_t i) {
        return (t.scalar == compute::Scalar::f32) ? static_cast<float*>(t.data)[i] : __bfloat162float(static_cast<__nv_bfloat16*>(t.data)[i]);
    }
    __device__ inline void write(compute::TensorView t, std::size_t i, float x) {
        if ((t.scalar == compute::Scalar::f32)) static_cast<float*>(t.data)[i] = x;
