...
namespace genesia::convnext {
    __global__ void ln_kernel(compute::TensorView out, compute::TensorView x, const float* weight, const float* bias, float* stats) {
...
#pragma unroll
    ...
    __global__ void ln_dx_kernel(compute::TensorView dx, compute::TensorView dy, compute::TensorView x, const float* weight, const float* stats) {
    ...
    __global__ void ln_param_kernel(compute::TensorView dy, compute::TensorView x, const float* stats, float* dw, float* db) {
    ...
    __global__ void grn_partial(compute::TensorView x, float* scratch) {
        int c = blockIdx.x * 128 + threadIdx.x;
        if (c >= x.c) return;
    ...
    __global__ void grn_stats(compute::TensorView x, const float* partial, float* stats, int parts) {
    ...
    __global__ void grn_apply(compute::TensorView out, compute::TensorView x, const float* weight, const float* bias, const float* stats) {
    ...
    __global__ void grn_backward_partial(compute::TensorView dy, compute::TensorView x, const float* stats, float* dw, float* db, float* partial) {
        int c = blockIdx.x * 128 + threadIdx.x;
        if (c >= x.c) return;
    ...
    __global__ void grn_backward_stats(compute::TensorView x, const float* weight, const float* stats, float* partial, int parts) {
        __shared__ cub::BlockReduce<float, 256>::TempStorage shared;
    ...
    __global__ void grn_dx(compute::TensorView dx, compute::TensorView dy, compute::TensorView x, const float* weight, const float* stats, const float* correction) {
    ...
    void layer_norm(cudaStream_t s, compute::TensorView o, compute::TensorView x, const float* w, const float* b, float* stats) {
        ln_kernel<<<(x.n * x.h * x.w + 3) / 4, 128, 0, s>>>(o, x, w, b, stats);
    }
    void layer_norm_backward(cudaStream_t s, compute::TensorView dx, compute::TensorView dy, compute::TensorView x, const float* w, const float* stats, float* dw, float* db, float*) {
        ln_dx_kernel<<<x.n * x.h * x.w, 256, 0, s>>>(dx, dy, x, w, stats);
        ln_param_kernel<<<dim3((x.c + 127) / 128, 128), 128, 0, s>>>(dy, x, stats, dw, db);
    }
    void grn(cudaStream_t s, compute::TensorView o, compute::TensorView x, const float* w, const float* b, float* stats, float* scratch) {
    ...
    void grn_backward(cudaStream_t s, compute::TensorView dx, compute::TensorView dy, compute::TensorView x, const float* w, const float* stats, float* dw, float* db, float* scratch) {
