...
namespace genesia::convnext {
    ...
    __global__ void preprocess_kernel(compute::TensorView out, const unsigned char* rgb, std::size_t stride, std::size_t image_stride, const Augment* a, bool training) {
    ...
    void preprocess(cudaStream_t s, compute::TensorView o, const unsigned char* rgb, std::size_t stride, std::size_t image_stride, Augment* a, RandomState* r, bool train) {
        if (train) {
