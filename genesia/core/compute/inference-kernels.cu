...
namespace genesia::compute::kernels {
    ...
    __global__ void convert_kernel(Output* output, const Input* input, const std::size_t count) {
    ...
    __global__ void geglu_kernel(__half* output, const __half* input, const __half* bias, const int count, const int width) {
        ...
        const float x    = float(input[row * width * 2 + column + width]) + float(bias[column + width]);
        const float y    = float(input[row * width * 2 + column]) + float(bias[column]);
        output[i]        = __half(y * (0.5F * x * (1.0F + erff(x * 0.7071067811865476F))));
    }
    ...
    template <class T, int Width, bool Residual = false>
    __global__ void layer_norm_kernel(T* output, const T* input, T* residual, const T* weight, const T* bias, const float epsilon) {
...
#pragma unroll
        for (int j = 0; j < (width + 255) / 256; ++j) {
            ...
            if constexpr (Residual)
                if (c < width) values[j] += float(residual[i]);
            sum += values[j];
        }
...
#pragma unroll
    ...
    template <class T, int GroupWidth>
    __global__ void group_moments_kernel(float* statistics, const T* input, const T* second, T* joined, const T* time, const int* step, const int spatial, const int first_width, const int chunks) {
...
#pragma unroll
            for (int j = 0; j < 8 * Columns; ++j)
                if (row + j / Columns < spatial && lane + (j % Columns) * 8 < GroupWidth) variance = fmaf(values[j] - mean, values[j] - mean, variance);
    ...
    template <class T>
    __global__ void group_output_kernel(T* output, const T* input, const T* weight, const T* bias, const float* statistics, const int count, const int spatial, const int width, const int chunks, const bool silu, const T* time, const int* step, const int batch) {
        const int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= count) return;
        const int c           = i % width;
    ...
    template <class T>
    __global__ void resize_kernel(T* output, const T* input, const int height, const int width, const int channels, const int count) {
        const int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= count) return;
    ...
    void geglu(const ::cuda::stream_ref stream, void* output, const void* input, const void* bias, const int rows, const int width) {
        const int count = rows * width * 4;
